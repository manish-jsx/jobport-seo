(function ($) {
 ...
 ====================================== */
  ...
  $(document).ready(function () {
    // if ( $( "#admin_menu_hider" ).length ) {
    //     $('body').css('padding-top', '0px');
    //     $(".navbar-top").css('padding-top', '0px');
    //     $('#toolbar-bar').css('display','none');
    // }

    if ($("#admin_menu_hider").length) {
      $("#admin_menu_hider").click(function () {
        if ($("#toolbar-bar").css("display") == "none") {
          $("body").css("padding-top", "79px");
          $(".navbar-top").css("padding-top", "79px");
          $("#toolbar-bar").css("display", "block");
          $("#admin_menu_hider").html("Hide Admin");
        } else {
          $("body").css("padding-top", "0px");
          $(".navbar-top").css("padding-top", "0px");
          $("#toolbar-bar").css("display", "none");
          $("#admin_menu_hider").html("Show Admin");
        }
      });
    }

    var data = [];
    $(".roadman_item").each(function (index) {
      data.push({
        date: $(this).attr("data-year"),
        content: $(this).attr("data-event"),
      });
    });

    $("#backfills-roadmap").roadmap(data, { eventsPerSlide: data.length });

    $("#search-form #edit-submit").click(function () {
      var searchTerm = $("#edit-keys").val();
      window.location.href = "/search/node?keys=" + searchTerm;
    });
  });
})(jQuery);
